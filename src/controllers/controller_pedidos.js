...
const create = (req, res) => {
    const {idcliente, idproduto, quantidade, datalancamento, total} = req.body;
    con.query('INSERT INTO pedidos (idcliente, idprod, idtelefone, datalancamento, total) VALUES (?, ?, ?, ?, ?)',
        ...
        (err, result) => {
            ...
            return res.status(201).json({ id: result.insertId, idcliente: result.idcliente, idprod: result.idprod, idtelefone: result.idtelefone, datalancamento: result.iddatalancamento, total: result.total});
        });
}
const read = (req, res) => {
